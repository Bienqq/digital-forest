...
export default function configureHttp(Vue) {
    ...
    const instance = axios.create({baseURL: process.env.VUE_APP_API_BASE_URL})

    // function with automatic refresh token when 401 Unauthorized error occurs
    const refreshAuthLogic = failedRequest => instance 
        .post(process.env.VUE_APP_API_REFRESH_TOKEN_URL, { refreshToken: localStorage.getItem("refreshToken")})
